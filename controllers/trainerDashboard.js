...
const trainerDashboard = {
  index(request, response) { //---------------------------------------------------------------> index method, called when ‘/ dashboard’ request received
    ...
    const loggedInTrainer = accounts.getCurrentTrainer(request); //---------------------------> finds which member is currently logged in
  ...
  updateComment(request, response) { //-------------------------------------------------------> updateComment method, called when ‘/ dashboard’ request received
    ...
    const assessmentToUpdate = memberStore.getAssessmentById(memberId, assessmentId); //------>
    assessmentToUpdate.comment = comment; //-------------------------------------------------->
    memberStore.save(); //-------------------------------------------------------------------->
    response.redirect('/trainerDashboard'); //------------------------------------------------>
  },
  
  addClass(request, response) { //------------------------------------------------------------> addClass method, called when ‘/ dashboard’ request received
  ...
  viewClass(request, response) { //----------------------------------------------------------->
    logger.info('showing class'); //---------------------------------------------------------->
    response.render('/viewclass'); //--------------------------------------------------------->
  },
  
  allClasses(request, response) { //----------------------------------------------------------> allClasses method, called when ‘/ dashboard’ request received
    logger.info("rendering all classes"); //-------------------------------------------------->
    const loggedInTrainer = accounts.getCurrentTrainer(request); //--------------------------->
    const list = loggedInTrainer.classes; //-------------------------------------------------->
